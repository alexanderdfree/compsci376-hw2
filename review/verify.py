"""Verify the corrected homework against the preserved pre-correction snapshot.

Run from any directory with the project Python environment. Results are written
to the ignored .review-work/ directory, leaving the saved evidence unchanged.
The isolated-install entry checks archived evidence; it does not reinstall NLTK.
"""
...
quotes = [line.lstrip()[2:].strip() for line in answers.splitlines() if line.lstrip().startswith('> ') and '…' not in line]
...
fresh = json.loads((REVIEW / 'evidence/corrections/fresh_setup.json').read_text())
check('recorded_isolated_fresh_install', fresh == {'exit_code': 0, 'punkt_tab_installed': True, 'generated_baseline_successfully': True})

changed = [str(p.relative_to(SRC)) for relative in sorted(p.relative_to(BEFORE) for p in BEFORE.rglob('*') if p.is_file())
           for p in [SRC / relative] if p.is_file() and
